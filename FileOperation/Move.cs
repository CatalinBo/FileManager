...
{
    public class Move(IFileSystem fileSystem, IProgressBar progressBar) : IIOOperation
    {
        ...
        private void MoveEntry(IFileInfo fileInfo)
        {
            try
            {
                var destinationPath = Path.Combine(progressBar.TargetPath!, fileInfo.Name);
                fileSystem.File.Move(fileInfo.FullName, destinationPath, false);
        ...
        private void MoveEntry(IDirectoryInfo directoryInfo, string destinationPath)
        {
